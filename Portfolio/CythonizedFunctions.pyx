...
def cython_get_ticker_unit_gradients(double[:, :] closing_values, double[:] current_portfolio_close,
...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_dsigma_db(double[:] combined_close, double[:, :] dr_db, double[:] dmu_db):
...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_dr_db(double[:, :] closing_values, double[:] combined_close, double[:] current_ticker_units):
    cdef int n = closing_values.shape[0]
    cdef int num_tickers = closing_values.shape[1]
...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_combined_close(double[:, :] closing_values, double[:] current_ticker_units,
...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_close_ratio(double[:, :] closing_values, double[:] combined_close):
    cdef int n = closing_values.shape[0]
    cdef int num_tickers = closing_values.shape[1]
